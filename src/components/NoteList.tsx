...
const NoteList: React.FC<NoteListProps> = ({ notes, onEdit, onDelete, onToggleFavorite }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {notes.map((note) => (
                <div key={note.id} className="bg-white dark:bg-gray-700 rounded-lg shadow-md p-6 flex flex-col justify-between">
                    <div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{note.title}</h3>
                        <p className="text-gray-700 dark:text-gray-200 text-sm mb-4 line-clamp-4">{note.content}</p>
                        {note.tags.length > 0 && (
